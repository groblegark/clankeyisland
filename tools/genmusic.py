...
def main():
    ...
    for name, build, bars, bpm, loop in [
            ...
            ("cityhall", build_march, MARCH_BARS, MARCH_BPM,
             MARCH_LOOP_BEATS),
            ("dynamohum", build_dynamohum, DHUM_BARS, DHUM_BPM,
             DHUM_LOOP_BEATS),
            ("dynamohum_lift", build_dynamohum_lift, DHUM_BARS, DHUM_BPM,
             DHUM_LOOP_BEATS)]:
        path = os.path.join(OUTDIR, f"{name}.mid")
        build().save(path)
...
if __name__ == "__main__":
    import sys
    # `emit-dynamo` emits ONLY Scene 10's MIDIs (the safe path -- main()'s
    # build_rag has a pre-existing crash on some mido versions, NOTES.md).
    if len(sys.argv) > 1 and sys.argv[1] == "emit-dynamo":
        emit_dynamohum()
    else:
        main()
